...
const MenuButtonStyle = (activePage : string, page : string) : string => {
    let globalStyle : string =
        ...
        ' rounded-lg flex flex-col gap-[1vw] items-center transition hover:-translate-y-[1vh] hover:scale-150 hover:ease-in-out hover:duration-200 text-'+page+' '
...
const ImgButtonStyle = (activePage : string, page : string) : string => {
    let globalStyle : string =
        ' rounded-full w-3/4 aspect-square overflow-hidden border-[0.2vw] bg-secondary bg-opacity-80 shadow-3xl bg-'+page+' active:scale-75 '
    ...
    return (
        <NavLink to={'/'+props.page} className={MenuButtonStyle(props.activePage,props.page)}>
            <div className={ImgButtonStyle(props.activePage,props.page)}>
                <img src={'assets/img/'+props.page+'.png'} className="w-[4vw] scale-125" alt={props.page+' photo'}/>
            </div>
...
const Navigation = (props : NavigationProps) => {
    return (
        <div className="flex justify-center">
            <div className="py-[3vh] px-[1vw] rounded-full shadow-3xl transition ease-in-out backdrop-blur-xl">
